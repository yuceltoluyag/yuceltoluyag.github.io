"use strict";
!(function () {
    const n = "-WNw5YpxvCMIj8LDM0bScg",
        e = `https://webmention.io/api/mentions.jf2?token=${n}&per-page=20`;
    function t() {
        const t = document.createElement("div");
        function a(n) {
            return n.includes("localhost:8000")
                ? n.replace("http://localhost:8000", "https://yuceltoluyag.github.io")
                : n;
        }
        t.className =
            "fixed bottom-5 right-5 w-11/12 max-w-md max-h-[80vh] overflow-y-auto bg-base-100 rounded-xl shadow-xl p-5 z-50";
        const s = document.createElement("h3");
        (s.textContent = "Webmention Test Aracı"),
            (s.className = "text-xl font-semibold mb-4 pb-2 border-b border-base-200 text-primary"),
            t.appendChild(s);
        const o = document.createElement("div");
        o.className = "mb-5";
        const c = document.createElement("label");
        (c.textContent = "Test edilecek URL:"),
            (c.className = "block mb-2 text-sm font-medium opacity-70"),
            o.appendChild(c);
        const l = document.createElement("input");
        (l.type = "text"),
            (l.value = a(window.location.href.split("#")[0].split("?")[0])),
            (l.className = "input input-bordered w-full mb-3"),
            o.appendChild(l);
        const r = document.createElement("div");
        (r.className = "mb-4 text-xs opacity-70"),
            window.location.href.includes("localhost") &&
                (r.innerHTML = `\n                <div class="alert alert-warning alert-xs p-2">\n                    <div>\n                        <iconify-icon icon="tabler:info-circle" class="mr-1"></iconify-icon>\n                        <span>Localhost ortamında test yapıyorsunuz. URL otomatik olarak production adresine çevrildi.</span>\n                    </div>\n                </div>\n                <div class="mt-1 text-xs">Production URL: <span class="text-primary">${a(
                    window.location.href
                )}</span></div>\n            `),
            o.appendChild(r);
        const d = document.createElement("div");
        d.className = "flex gap-2 flex-wrap mb-5";
        const m = document.createElement("button");
        (m.textContent = "Bu URL için Göster"), (m.className = "btn btn-primary btn-sm flex-1"), d.appendChild(m);
        const p = document.createElement("button");
        (p.textContent = "Tüm Webmention'lar"),
            (p.className = "btn btn-success btn-sm flex-1"),
            d.appendChild(p),
            o.appendChild(d),
            t.appendChild(o);
        const v = document.createElement("div");
        (v.className = "bg-base-200 rounded-md p-3 max-h-[50vh] overflow-y-auto"), t.appendChild(v);
        ...
        (h.innerHTML = "&times;"),
            ...
            m.addEventListener("click", () => {
                !(function (e, t) {
                    ...
                    const a = `https://webmention.io/api/mentions.jf2?target=${encodeURIComponent(e)}&token=${n}`;
                    fetch(a)
                        .then((n) => n.json())
                        .then((n) => {
                            i(n, t);
                ...
                !(function (n) {
                    (n.innerHTML =
                        ...
                        fetch(e)
                            ...
                            .catch((e) => {
                                (n.innerHTML = `\n                    <div class="alert alert-error shadow-sm">\n                        <div class="flex gap-2 items-center">\n                            <iconify-icon icon="tabler:alert-triangle" width="24" height="24" class="animate-pulse"></iconify-icon>\n                            <div>\n                                <div class="font-medium">İşlem Başarısız</div>\n                                <div class="text-xs opacity-80">Hata: ${e.message}</div>\n                            </div>\n                        </div>\n                    </div>\n                `),
                                    console.error("Webmention API hatası:", e);
                            });
                })(v);
            }),
        ...
        t.forEach((n) => {
            i += `\n                <div class="card bg-base-100 shadow-sm hover:shadow-md transition-all duration-300">\n                    <div class="card-body p-4">\n                        <div class="flex items-center gap-3">\n                            ${
                ...
                n.author && n.author.name
                    ? `<div class="font-medium">${n.author.name}</div>`
                    : '<div class="font-medium">Anonim</div>'
            }\n                                ${
                n.published
                    ? `<div class="text-xs opacity-60">${new Date(n.published).toLocaleString("tr-TR", {
                          year: "numeric",
                          month: "long",
                          day: "numeric",
                      })}</div>`
                ...
                n.content && n.content.text
                    ? `<div class="mt-3 p-3 bg-base-200 rounded-md text-sm">${n.content.text}</div>`
                    : ""
    ...
    console.log("Webmention API Test Aracı"),
        console.log("-------------------------"),
        console.log("API Endpoint:", e),
        window.addEventListener("load", function () {
            const n = document.createElement("div");
            n.className = "fixed bottom-5 right-5 z-50";
            const e = document.createElement("button");
            (e.className = "btn btn-primary btn-sm gap-2 shadow-md"),
                (e.innerHTML =
