...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const n = document.querySelector("#mastodon-metabox");
    if (!n) return;
    const t = document.createElement("div");
    (t.id = "mastodon-id-finder"),
        (t.className = "card bg-base-100 shadow-xs mt-4"),
        (t.innerHTML =
        ...
        n.appendChild(t);
    const e = document.getElementById("extract-mastodon-id"),
        o = document.getElementById("copy-mastodon-id"),
        a = document.getElementById("mastodon-url-input"),
        s = document.getElementById("mastodon-result"),
        i = document.getElementById("mastodon-id-result"),
        l = document.getElementById("mastodon-meta-result");
    e.addEventListener("click", function () {
        const n = a.value.trim();
        if (n)
            try {
                const t = new URL(n),
                    e = t.pathname.split("/").filter(Boolean);
                if (e.length < 2) throw new Error("Geçersiz Mastodon URL formatı");
                const a = e[e.length - 1];
                e[e.length - 2].replace("@", ""),
                    t.hostname,
                    (i.value = a),
                    (l.value = `Mastodon_Link: ${n}`),
                    s.classList.remove("hidden"),
                    o.removeAttribute("disabled");
            } catch (n) {
                alert("Geçersiz URL: " + n.message);
            }
        else alert("Lütfen bir Mastodon URL'si girin");
    }),
        o.addEventListener("click", function () {
            ...
            navigator.clipboard
                ...
                .then(function () {
                    const n = o.innerHTML;
                    (o.innerHTML = '<iconify-icon icon="tabler:check" class="mr-2"></iconify-icon> Kopyalandı!'),
                        setTimeout(function () {
                            o.innerHTML = n;
                        }, 2e3);
                })
                .catch(function (n) {
                    alert("Kopyalama hatası: " + n);
                });
        });
});
