...
def interlink_translations(content):
    """Safely link content to its translations, skipping missing native URLs."""
    lang = content.lang
    ...
    for translation in content.translations:
        ...
        try:
            ...
            _LOGGER.warning(
                f"[i18n_subsites] Missing native URL for translation: "
                f"{translation.source_path} (from {content.source_path}). "
                "Skipping link."
            )
            continue
...
def interlink_removed_content(generator):
    """For all contents removed from generation queue update interlinks safely."""
    current_lang = generator.settings["DEFAULT_LANG"]
    for content in _GENERATOR_DB.get(generator, []):
        try:
            url = _NATIVE_CONTENT_URL_DB[content.source_path]
        except KeyError:
            _LOGGER.warning(
                f"[i18n_subsites] Missing native URL for {content.source_path}. "
                "This usually means the base translation wasn't processed yet."
            )
            continue
...
def create_next_subsite(pelican_obj):
    ...
    if len(_SUBSITE_QUEUE) == 0:
        _LOGGER.debug("i18n: All subsites generated. Populating _NATIVE_CONTENT_URL_DB for all content.")
        # Populate _NATIVE_CONTENT_URL_DB for all content from all generators
        for generator_instance in _GENERATOR_DB:
            inspector = GeneratorInspector(generator_instance)
            for content in inspector.all_contents():
                # Store the URL of the content in its native language
                # We assume content.url is correctly set at this point for all content
                _NATIVE_CONTENT_URL_DB[content.source_path] = content.url
        ...
        with temporary_locale():
            ...
            # After running the subsite generator, populate _NATIVE_CONTENT_URL_DB
            # with the URLs of its native content.
            if hasattr(new_pelican_obj, "generators") and new_pelican_obj.generators:
                for generator_instance in new_pelican_obj.generators:
                    inspector = GeneratorInspector(generator_instance)
                    for content in inspector.all_contents():
                        if content.lang == lang:
                            ...
                            _LOGGER.warning(
                                "[i18n_subsites] Pelican object has no 'generators' attribute. "
                                "Skipping population of _NATIVE_CONTENT_URL_DB for this subsite."
                            )
