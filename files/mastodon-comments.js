...
class MastodonComments extends HTMLElement {
    ...
    toot_stats(toot) {
      ...
      <div class="replies ${this.toot_active(toot, "replies")}">
        <a href="${toot.url}" rel="nofollow"><i class="fa fa-reply fa-fw"></i>${this.toot_count(toot, "replies")}</a>
      </div>
      <div class="reblogs ${this.toot_active(toot, "reblogs")}">
        <a href="${toot.url}" rel="nofollow"><i class="fa fa-retweet fa-fw"></i>${this.toot_count(toot, "reblogs")}</a>
      </div>
      <div class="favourites ${this.toot_active(toot, "favourites")}">
        <a href="${toot.url}" rel="nofollow"><i class="fa fa-star fa-fw"></i>${this.toot_count(toot, "favourites")}</a>
      </div>
    `;
    }
    ...
    render_toot(toots, toot, depth) {
        ...
        <div class="author">
          <div class="avatar">
            <img src="${this.escapeHtml(toot.account.avatar_static)}" height=60 width=60 alt="">
          </div>
          <div class="details">
          ...
          <a class="date" href="${toot.url}" rel="nofollow">${toot.created_at.substr(0, 10)} ${toot.created_at.substr(
            11,
            8
        )}</a>
        </div>
        ...
        div.innerHTML =
            typeof DOMPurify !== "undefined" ? DOMPurify.sanitize(mastodonComment.trim()) : mastodonComment.trim();
