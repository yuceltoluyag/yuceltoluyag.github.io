...
export function copy_media_dev() {
  return gulp.src(configDev.copy.media.src).pipe(gulp.dest(configDev.copy.media.dest));
}
...
export function copy_fonts_prod() {
  return gulp.src(configProd.copy.fonts.src).pipe(gulp.dest(configProd.copy.fonts.dest));
}
...
export const build_dev = gulp.series(
  clean_dev,
  jekyll_build_dev,
  gulp.parallel(styles_dev, scripts_dev, copy_dev, convert_media_webp)
);
...
function jekyll_build_dev(cb) {
  ...
  spawn('bundle', args, { stdio: 'inherit' }).on('close', cb);
}
