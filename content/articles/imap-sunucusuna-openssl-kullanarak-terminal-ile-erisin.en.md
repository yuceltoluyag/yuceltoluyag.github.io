Title: Accessing IMAP Server with OpenSSL via Terminal
...
Open the terminal and enter the following command. I shared the IMAP addresses and ports in the first part.

```bash
openssl s_client -crlf -connect imapaddress:imapport
```
...
When you establish the connection, the output will be as follows:

```bash
openssl s_client -crlf -connect imap.gmail.com:993 #output when we establish the connection is as below
CONNECTED(00000003)
...
# at this point it waits for your email address and password.
...
```bash
tag login yourgmailaddress@gmail.com yourpassword
```

After connecting, you can use the following command to see mailboxes:

```bash
tag LIST "" "\*"
```

With this command you can see the remote names and folder structure. Since I use the Turkish interface, the list appears as follows:
...
For example, if I want to access sent emails within muttrc, I should write **[Gmail]/G&APY-nderilmi&AV8- Postalar** in the remote part. If I write an incorrect remote tag, when I start neomutt, it gives an error like **slave sent cannot be opened**. You can skip this error and connect to the interface but you cannot access the sent emails part. <span class="text-highlight">I'm repeating REMOTE tag is very important.</span>

### Hotmail/Outlook Access

Let's also look at Outlook/Hotmail, how are the folder structure and tags
...
```bash
tag login myhotmailaddress.com yourpassword
```
...
This way you can get the remote tags of all email interfaces that you know the imap and port addresses of.

## Extra Information

You can select the email box
...
You can see how many messages are in the selected box

```bash
tag STATUS INBOX (MESSAGES)
```

Getting the headers of the message
Let's assume the result of the above operation is 15. Let's get the headers of the last 10 emails
...
If we only want to retrieve the content parts

```bash
tag FETCH 15 (BODY)
```

Since the content parts are multipart (happens in reply to reply situations), if we want to see the index numbers
