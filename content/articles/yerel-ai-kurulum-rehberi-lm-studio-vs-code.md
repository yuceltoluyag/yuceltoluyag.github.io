...
Bugün size bu işin "Masterclass" kıvamında detaylarını anlatacağım. Donanımınız ne olursa olsun, bu işin mantığını kavradığınızda her sistemde akıcı bir AI çalıştırabilirsiniz.

## 🧠 Bölüm 1: Yerel AI Nasıl Çalışır? (VRAM Meselesi)

Bir modelin performansını belirleyen iki ana unsur vardır: **Parametre sayısı** (beyin büyüklüğü) ve **Context Size** (hafıza kapasitesi). 862 milyar parametrelik bir canavarı evdeki bilgisayarda koşturamazsınız, ama doğru optimize edilmiş modellerle harikalar yaratabilirsiniz.

İşin sırrı ekran kartınızdaki (GPU) **VRAM** miktarında yatıyor. ⚡
...
2.  **Model Ara:** Arama kısmına `Qwen` veya `Llama` yazın. 
...
Verileriniz bilgisayarınızda kalsın, cüzdanınızdaki para da cebinizde. Kendi yerel AI masterclass'ınızı başlatmak için bugünden daha iyi bir zaman yok.
